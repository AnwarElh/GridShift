...
const index = [
  { kind: 'game', title: 'Echo Divide', sub: 'Northlight', href: '/jeux/echo-divide/' },
  { kind: 'guide', title: 'Bien démarrer sans se bloquer', sub: 'Echo Divide', tags: ['débutant'], href: '/g/1/' },
  { kind: 'review', title: 'Iron District tient sa promesse', sub: 'Ninefold', href: '/t/1/' },
  { kind: 'news', title: 'Le patch 4.2a corrige l’économie', sub: 'Echo Divide', href: '/a/1/' },
];
...
test('les groupes suivent l’ordre du menu et ignorent les vides', () => {
  ...
  assert.deepEqual(g.map((x) => x.kind), ['game', 'guide', 'news']);
});
